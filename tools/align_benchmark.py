...
# VAD-based segmentation -> transcribe per-segment and concatenate

def vad_split_audio(path: str, sr_target: int = 16000, frame_ms: int = 30, hop_ms: int = 10, energy_thresh: float | None = None):
    import numpy as np
    data, sr = sf.read(path, dtype='float32')
    if data.ndim > 1:
        data = data.mean(axis=1)
    ...
    if sr != sr_target:
        tensor = torch.from_numpy(data).unsqueeze(0)
        tensor = F.resample(tensor, sr, sr_target)
        data = tensor.squeeze(0).numpy()
        sr = sr_target
    ...
    # pad
    import math
    n_frames = max(1, 1 + (len(data) - frame_len) // hop_len)
    energies = []
    for i in range(n_frames):
        start = i * hop_len
        frame = data[start:start+frame_len]
        energies.append(float((frame**2).mean()) if frame.size else 0.0)
    energies = np.array(energies)
    if energy_thresh is None:
        # set threshold relative to median
        energy_thresh = max(energies.mean() * 0.5, np.median(energies) * 0.5)
    voiced = energies > energy_thresh
    # merge contiguous voiced frames into segments
    segments = []
    in_seg = False
    for i, v in enumerate(voiced):
        if v and not in_seg:
            ...
            seg_end = min(len(data), i * hop_len + frame_len)
            segments.append((seg_start, seg_end))
            in_seg = False
    if in_seg:
        segments.append((seg_start, len(data)))
    # filter very short
    min_dur = int(sr * 0.5)
    filtered = [(s,e) for s,e in segments if (e-s) >= min_dur]
    # convert to seconds
    return [(s/sr, e/sr) for s,e in filtered]


def transcribe_segments_and_collect(path, segments_seconds):
    merged_segments = []
    for start_s, end_s in segments_seconds:
        ...
        # resample to 16000
        if sr != 16000:
            tensor = torch.from_numpy(slice_data).unsqueeze(0)
            tensor = F.resample(tensor, sr, 16000)
            slice_data = tensor.squeeze(0).numpy()
            sr = 16000
        # whisper transcribe
        res = model.transcribe(slice_data, word_timestamps=True)
        segs = res.get('segments', [])
        # adjust start offsets
        for s in segs:
            s['start'] = s.get('start',0.0) + start_s
            s['end'] = s.get('end',0.0) + start_s
            for w in s.get('words', []):
                if 'start' in w:
                    w['start'] = w.get('start',0.0) + start_s
                if 'end' in w:
                    w['end'] = w.get('end',0.0) + start_s
        merged_segments.extend(segs)
    return merged_segments

print('\nRunning VAD-segmented transcription...')
segs = vad_split_audio(audio_path)
print('VAD segments (s):', segs)
if segs:
    vad_segments = transcribe_segments_and_collect(audio_path, segs)
    print('Collected segments from per-segment transcription:', len(vad_segments))
    # Eval baseline/fuzzy on vad_segments
    vad_baseline_pred = greedy_align(plain_lines, vad_segments)
    vad_baseline_eval = eval_pred(gt, vad_baseline_pred)
    print('VAD Baseline:', vad_baseline_eval)
    vad_fuzzy_pred = fuzzy_align(plain_lines, vad_segments)
    vad_fuzzy_eval = eval_pred(gt, vad_fuzzy_pred)
    print('VAD Fuzzy:', vad_fuzzy_eval)
else:
    print('No VAD segments found; skipping VAD variants')

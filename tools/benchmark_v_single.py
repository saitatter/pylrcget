#!/usr/bin/env python3
"""Benchmark single version with local parameters."""
...
_worker_cls = getattr(ai_module, 'AiSyncWorker', None)
...
for track_info in tracks:
    ...
    # Check if current version has section anchors
    section_anchors = None
    if _worker_cls and hasattr(_worker_cls, '_detect_audio_section_anchors'):
        section_anchors = _worker_cls._detect_audio_section_anchors(audio_np, 16000)
    
    # Call alignment function
    print(f'  Running alignment...')
    if _align:
        # Current version (with Viterbi support)
        try:
            raw_lrc = _align(
                plain_lines,
                segments,
                ...
                repeat_penalty=50.0,
                max_backtrack=0.5,
                enable_viterbi=True,  # Auto-selector includes viterbi
                min_repeat_gap=4,
                section_anchors=section_anchors,
            )
        except TypeError:
            # Fallback for release version (no viterbi support)
            raw_lrc = _align(
                plain_lines,
                segments,
            )
    else:
        print('  ERROR: Could not find _align_lyrics_to_segments')
        continue
    
    pred = parse_lrc_output(raw_lrc)
    eval_result = evaluate(gt, pred)
    
    results[name] = {
        'gt_count': len(gt),
        'fuzzy_local': eval_result,
        'auto_viterbi': eval_result,  # For current version
    }
    
    print(f'  Result: mean={eval_result["mean_abs_s"]:.2f}s, median={eval_result["median_abs_s"]:.2f}s, p95={eval_result["p95_abs_s"]:.2f}s')
...
for name, res in results.items():
    mean = res['fuzzy_local']['mean_abs_s'] or 999
    print(f'{name:40s} mean={mean:6.2f}s')

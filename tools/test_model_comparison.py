#!/usr/bin/env python3
"""
Investigation 2: Test Whisper base vs large-v3 model quality/stability.

Goal: Understand if better ASR model = better timestamp reliability.
Run both models on same audio, compare alignment accuracy.
"""
import json
import sys
import time
...
def benchmark_models(track_name: str) -> dict:
    """Benchmark Whisper base vs large-v3."""
    print(f"\n{'='*70}")
    ...
    for model_name in ["base", "large-v3"]:
        ...
        print(f"    Load time: {load_time:.1f}s, Transcribe time: {transcribe_time:.1f}s")
...
if __name__ == "__main__":
    ...
    for track in tracks:
        try:
            all_results[track] = benchmark_models(track)
    ...
    # Summary
    print(f"\n{'='*70}")
    print("SUMMARY - Base vs Large-v3")
    print(f"{'='*70}")
    ...
    # Save results
    output_path = Path(__file__).parent / "whisperx_test_output" / "model_comparison_base_vs_large.json"

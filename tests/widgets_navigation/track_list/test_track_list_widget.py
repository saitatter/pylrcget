...
class TrackListWidgetTests(unittest.TestCase):
    ...
    def test_context_menu_exposes_artist_and_album_navigation(self):
        app_state = simple_app_state()
        widget = TrackListWidget(app_state)
        routes = []
        try:
            ...
            self.assertEqual(actions["open_artist"].text(), "Open artist")
            self.assertEqual(actions["open_album"].text(), "Open album")
            ...
            self.assertEqual(routes[-1], artists_detail((7,)))
        finally:
            widget.deleteLater()
            app_state.db.close()
    ...
    def test_track_table_keeps_lyrics_status_column_visible(self):
        ...
        try:
            ...
            self.assertEqual(widget.table.verticalHeader().defaultSectionSize(), 44)
        finally:
            widget.deleteLater()
            app_state.db.close()
